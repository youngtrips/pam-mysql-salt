pam_mysql - A PAM authentication module against MySQL database with salt option support, modified by Tuz(youngtrips@gmail.com).
...
saltcolumn
    ...
    The name of the column that contains salt string, only support by md5 method.
    The encrypt expression is:

        final_pass = md5(md5(sent_pass)..salt)
