 ...
 * Add md5 salt support by: Tuz <youngtrips@gmail.com>
 *
...
static pam_mysql_option_t pam_mysql_entry_handler_options[] = {
    ...
    PAM_MYSQL_DEF_OPTION2(users.salt_column, saltcolumn, &pam_mysql_string_opt_accr),
			...
			switch (ctx->crypt_type) {
				...
				case 3: {
                    ...
                    if (ctx->saltcolumn == NULL) {
                        ...
                        char buf2[128];
                        sprintf(buf2, "%s%s", buf, row[1]);
                        pam_mysql_md5_data((unsigned char*)buf2, strlen(buf2), buf);
